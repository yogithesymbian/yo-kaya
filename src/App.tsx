...
import { Trash2, Edit3, Plus, Save, X, TrendingUp, ChevronLeft, ChevronRight, Calendar, DollarSign, Tag, FileText } from 'lucide-react';
...
// Simulasi API functions dengan data yang lebih banyak
const api = {
  async getExpenses(page: number = 1, limit: number = 10): Promise<{ data: Expense[], total: number, totalPages: number }> {
    ...
    if (stored.length === 0) {
      ...
      // const sorted = sampleData.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
...
const ExpenseTracker: React.FC = () => {
  ...
  const loadExpenses = async () => {
    setLoading(true);
    try {
      const result = await api.getExpenses(currentPage, 10);
